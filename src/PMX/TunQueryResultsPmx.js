...
// //   const [companyId, setCompanyId] = useState('12400620');
...
// //                   onKeyPress={handleKeyPress}
// //                   placeholder={employees.length > 0 ? "搜尋員工或點擊選擇" : "正在載入員工資料..."}
...
// //                     borderRadius: '4px',
// //                     fontSize: '14px',
// //                     backgroundColor: loadingEmployees ? '#f5f5f5' : 'white',
...
//     employee.employee_id !== 116 && employee.employee_id !== '116'
//   );
...
//   // 使用 useCallback 優化全選功能 - 移除員工數量限制
//   const handleSelectAll = useCallback(() => {
//     const allSelected = filteredEmployees.every(emp => 
//       selectedEmployees.some(selected => selected.employee_id === emp.employee_id)
//     );
...
//       setError('一次最多只能查詢100位員工，請分批查詢以確保系統穩定性');
//       setShowError(true);
//       return;
//     }
...
//                     fontStyle: 'italic'
//                   }}>
//                     {employees.length === 0 ? '沒有找到員工資料' : '找不到符合條件的員工'}
//                   </div>
//                 ) : (
//                   <div style={{
//                     display: 'grid',
...
//                               gap: '8px'
//                             }}>
//                               <span>工號: {employee.employee_id}</span>
...
//                 {employeeSearchQuery && ` (篩選後: ${filteredEmployees.length} 位)`}
//               </div>
//             </div>
...
const TunQueryResults = React.memo(() => {
  ...
  const contentRef = useRef(null);
  const modalRef = useRef(null);
  const API_URL = `${API_BASE_URL}/api/attendance-check-in-view`;
  const EMPLOYEES_API_URL = `${API_BASE_URL}/api/employees`;
  const APPROVED_APPLICATIONS_API_URL = `${API_BASE_URL}/api/attendance/approved-applications`;
  ...
  const exportSingleEmployeeToPdf = async (employeeId) => {
    ...
    try {
      ...
      let companyName = '';
      try {
        const companyResponse = await axios.get(`${API_BASE_URL}/api/employee`, {
          params: {
            company_id: companyId,
            employee_id: employeeId
          },
