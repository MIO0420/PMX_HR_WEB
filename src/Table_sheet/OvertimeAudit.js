...
// import styles from './Table_sheet_css/SharedAuditStyles.module.css'; // 使用相同的樣式表
...
//         </div>
//         <div className={styles.pageTitle}>加班申請單</div>
...
//             <div style={{ margin: '10px 0' }}>
//               <span style={{ color: '#666', fontSize: '14px' }}>加班類型：</span>
//               <span style={{ color: '#333', fontSize: '14px' }}>工作日加班</span>
//             </div>
            
//             <div style={{ margin: '10px 0' }}>
//               <span style={{ color: '#666', fontSize: '14px' }}>補償：</span>
//               <span style={{ color: '#333', fontSize: '14px' }}>加班費</span>
//             </div>
            
//             <div style={{ margin: '10px 0' }}>
//               <span style={{ color: '#666', fontSize: '14px' }}>加班時間起迄：</span>
//               <span style={{ color: '#333', fontSize: '14px' }}>
//                 {request.start_date} {formatTime(request.start_time)}
//               </span>
//             </div>
//             <div style={{ margin: '10px 0' }}>
//               <span style={{ color: '#666', fontSize: '14px', visibility: 'hidden' }}>加班時間起迄：</span>
//               <span style={{ color: '#333', fontSize: '14px' }}>
...
//               <span style={{ color: '#333', fontSize: '14px' }}>
//                 {request.total_calculation_hours || request.total_hours || "2"}小時{request.total_minutes || "0"}分鐘
//               </span>
//             </div>
            
//             <div style={{ margin: '10px 0' }}>
//               <span style={{ color: '#666', fontSize: '14px' }}>申請說明：</span>
//               <span style={{ color: '#333', fontSize: '14px' }}>{request.illustrate || "設計新功能介面"}</span>
//             </div>
...
//       if (result.Status !== "Ok") {
//         throw new Error(result.Msg || "獲取加班申請失敗");
//       }
...
//         application_time: item.application_time || "", // 添加時間字段
...
//       console.log('格式化後的加班申請數據:', formattedRequests);
...
//     // 🔥 顯示更詳細的成功訊息，包含修正打卡記錄的結果
//     let alertMessage = `已批准申請 ${formNumber}\n審核人: ${employeeName || employeeId}\n審核狀態: 已通過`;
    
//     if (result.Data && result.Data.attendance_status) {
//       alertMessage += `\n打卡記錄修正結果: ${result.Data.attendance_status.Msg || '已修正'}`;
//       if (result.Data.attendance_status.Data) {
//         alertMessage += `\n修正了 ${result.Data.attendance_status.Data} 筆出勤記錄`;
//       }
//     }
...
//       const requestData = {
//         company_id: companyId,
//         form_number: originalFormNumber, // 使用原始表單編號
//         action: "reject", // 固定為 reject
//         reviewer: employeeName || employeeId // 使用員工姓名，如果沒有則使用員工ID
//       };
...
//     }
//   }, [employeeId, companyId, statusFilter, fetchOvertimeRequests]);
...
//       <div className={styles.appWrapper}>
//         <div className={styles.header}>
//           <div className={styles.homeIcon} onClick={handleHomeClick || handleGoHome}>
//             <img src={homeIcon} alt="首頁" style={{ width: '20px', height: '20px' }} />
//           </div>
//           <div className={styles.pageTitle}>簽核加班單</div>
//           <div className={styles.timeDisplay}>{currentTime}</div>
//         </div>
...
//                 >
//                   <div className={styles.requestHeader} style={{ padding: '5px 10px' }}>
...
//                         加班日期：{request.start_date} 周{weekdayName} {formattedStartTime}~{formattedEndTime}
//                       </div>
//                     </div>
//                   </div>
//                 </div>
...
// // 加班功能內容組件 - 使用與 OvertimeAudit 相同的邏輯但可以接受不同的 props
...
// }) => {
//   // 這裡使用與 OvertimeAudit 相同的邏輯，但可以接受不同的 props
//   return <OvertimeAudit 
...
// export default OvertimeAudit;
// export { OvertimeContent };
import React, { useState, useEffect, useCallback } from "react";
import { useNavigate } from "react-router-dom";
import styles from './Table_sheet_css/SharedAuditStyles.module.css'; // 使用相同的樣式表
import homeIcon from '../Google_sheet/HomePageImage/homepage.png';
...
const OvertimeRequestDetail = ({ 
  ...
  styles,
  employeeNames,
  reviewerName // 🔥 新增 reviewerName 參數
}) => {
  // 根據申請單狀態設置顯示文字和顏色
  const getStatusDisplay = () => {
    if (request.status === "ok") {
...
const handleApprove = async (formNumber) => {
  try {
    ...
    const requestData = {
      category: "work_overtime", // 🔥 加班申請的類別
      reviewer: employeeName || employeeId // 🔥 新增核准人資訊
    };
  ...
  if (selectedRequest) {
    return (
      <OvertimeRequestDetail
        ...
        styles={styles}
        employeeNames={employeeNames} // 傳遞員工姓名對照表
        reviewerName={employeeName} // 🔥 傳遞當前登入用戶的姓名作為核准人
      />
    );
  }
                ...
                >
                  ...
                  <div className={styles.requestContent}>
                    ...
                    <div className={styles.requestDetails}>
                      ...
                      <div className={styles.requestInfo}>
                        加班日期：{request.start_date} 周{weekdayName} {formatTime(request.start_time)}~{formatTime(request.end_time)}
                      </div>
                    </div>
                  </div>
                </div>
...
export default OvertimeAudit;
export { OvertimeContent, Header, BackToSystemLink, StatusTabContainer };
